...
static const char *sf_pinctrl_state_names[SF_PIN_STATE_MAX] = {
    "power_on", "power_off", "finger_rst_clr", "finger_rst_set", "eint_set",
};
...
static long sf_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    xprintk(KERN_INFO, "%s(cmd = 0x%08x, ..)\n", __FUNCTION__, cmd);
...
static int sf_probe(struct spi_device *spi)
{
...
#else
    xprintk(KERN_INFO, "sf_ctl_init_irq begin\n");
    err = sf_ctl_init_irq();
    xprintk(KERN_INFO, "sf_ctl_init_irq end\n");
#endif
    ...
    xprintk(KERN_INFO, "misc_register begin\n");
    err = misc_register(&sf_ctl_dev.miscdev);
    xprintk(KERN_INFO, "misc_register end\n");
...
static int sf_ctl_init_gpio_pins(void)
{
	...
	printk("sf_ctl: entry sf_ctl_init_gpio_pins.\n");
    xprintk(KERN_INFO, "%s(..) enter.\n", __FUNCTION__);
...
static int sf_ctl_init_irq(void)
{
    ...
    /* Get the irq number. */
    dev_node = of_find_compatible_node(NULL, NULL, "mediatek,goodix_fp");
...
{
    int err = 0;
// add by xulp for compatile start
    int status_id = 0;
 //   int status_irq = 0;

    xprintk(KERN_ERR, "enter sunwave function : '%s'\n", __FUNCTION__);
    //status_id = gpio_get_value(25);   // vendor id gpio num
    //printk("enter sunwave '%s', status_id = %d \n", __func__, status_id);

	xprintk(KERN_ERR, "enter sunwave function");
	xprintk(KERN_INFO, "enter sunwave function");
	xprintk(KERN_DEBUG, "enter sunwave function");
	printk("enter sunwave finger successful");
    //status_irq = gpio_get_value(12);   // irq gpio num
    //printk("enter sunwave '%s', status_irq = %d \n", __func__, status_irq);
	xprintk(KERN_ERR, "ssssssssss");
#if 1
    if (status_id == 0) {
        printk("enter sunwave finger successful, status_id = %d \n", status_id);
    } else {
        printk("enter sunwave finger faile, status_id = %d \n", status_id);
	return -1;
    }
#endif
// add by xulp for compatile end
...
#endif
    xprintk(KERN_INFO, " sf_ctl_init_gpio_pins before err = %d.", err);
    err = sf_ctl_init_gpio_pins();
    xprintk(KERN_INFO, " sf_ctl_init_gpio_pins end err = %d.", err);

    if (err) {
        xprintk(KERN_ERR, "sf_ctl_init_gpio_pins failed with %d.\n", err);
        return err;
    }
    xprintk(KERN_INFO, " sf_ctl_init_gpio_pins before.");
    err = sf_ctl_device_power(true);
    xprintk(KERN_INFO, " sf_ctl_device_power end.");
    xprintk(KERN_INFO, " sf_ctl_device_reset before.");
    sf_ctl_device_reset();
    xprintk(KERN_INFO, " sf_ctl_device_reset end.");
